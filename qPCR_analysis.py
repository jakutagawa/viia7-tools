...
def generateStandards (mean_dict):
    ...
    panel2.plot(x_pos,hk_y_pos,alpha = 0.5, label='ACTB',color='orange')
...
def normalize (qPCR_excel):
    ...
    sample_dict = dict()
    norm_score_array = np.ones(len(qPCR_excel))
    ...
    for index, row in sorted_excel.iterrows():
        ...
        gene_score = sorted_excel.iloc[[count+len(qPCR_excel)/2]]['10^RVQ'].values
        normalized_score = gene_score / housekeeping_score

        norm_score_array[count+len(qPCR_excel)/2]=(float(normalized_score))
        count += 1
...
class CommandLine() :
    ...
    '''
        ...
        self.parser = argparse.ArgumentParser(description = 'Program prolog - Specify the <input and >output files, gene names, and standards.',
                                              epilog = 'Program epilog - parameters of infile and outfile must be given.',
                                              add_help = True, #default is True
                                              prefix_chars = '-',
                                              usage = '%(prog)s <infile >outfile -g | -h | -s'
                                              )
        ...
        self.parser.add_argument('-h', '--housekeepingGene', default='ACTB',
